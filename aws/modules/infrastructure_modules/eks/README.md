...
|------|-------------|------|---------|:--------:|
| <a name="input_block_device_mappings"></a> [block\_device\_mappings](#input\_block\_device\_mappings) | Specify volumes to attach to the instances. This is useful if you want to increase the volume where the container image layers are stored. Default is 20gb | `map(any)` | `{}` | no |
...
| <a name="input_node_iam_assume_role_policy"></a> [node\_iam\_assume\_role\_policy](#input\_node\_iam\_assume\_role\_policy) | Allowing you to override the assume role policy assigned to the node group IAM Role. If this is set then this module will create the IAM Role and attach this trust policy to it. | `any` | `null` | no |
