...
def create_run(
    adf_client, resource_group_name, factory_name, pipeline_name, parameters
):
...
def check_for_active_run(adf_client, resource_group_name, factory_name, pipeline_names):
    ...
    filter_params = RunFilterParameters(last_updated_after=datetime.now() - timedelta(1), last_updated_before=datetime.now() + timedelta(1), filters = [status_filter, pipeline_name_filter])
...
def run_adf_pipeline(
    pipeline_parameters, pipeline_name, activity_data_bool=False, activity_name_filter="create_incident", check_for_runs_bool=False, dependant_pipelines=None
):
    ...
    if check_for_runs_bool:
        ...
        existing_run_status, run_id = check_for_active_run(adf_client, resource_group_name, factory_name, dependant_pipelines)
    ...
    if activity_data_bool:
        ...
        filter_params = RunFilterParameters(last_updated_after=datetime.now() - timedelta(1), last_updated_before=datetime.now() + timedelta(1), filters=[activity_name])
        query_response = adf_client.activity_runs.query_by_pipeline_run(resource_group_name, factory_name, run_id, filter_params)
